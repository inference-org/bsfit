...
function output = SLfitBayesianModel(subjects,initp,varargin)
...
if ~slIsInput(varargin,'motionenergymodel')
    fprintf('%s \n','(SLfitBayesianModel) Fitting the Basic Bayesian motion energy model...')
end
...
%case initp parameters are rescaled to unit values
%use specified scaling factors
if slIsInput(varargin{1},'pscaling')
    %normalize the fit parameters to have the same units
    %improve the fit. note : the parameters must be divided
    %by the same scaling vector within the objective
    %function
    pscales = varargin{find(strcmp(varargin,'pscaling'))+1};
    output.pscales = pscales;
    output.initpInput = initp;
    output.initpAfterScaling = initp.*pscales;
    fprintf('%s \n','(SLfitBayesianModel) initp are:')
    display(initp)
    fprintf('%s \n','(SLfitBayesianModel) Check scaled params have same order of magnitude')
    display(initp.*pscales)
end
...
if ~slIsInput(vrg,'sherlock')
    if slIsInput(vrg,'vonMisesPrior')&&~slIsInput(vrg,'bimodalPrior')
        ...
        elseif slIsInput(vrg,'ui')
            ...
            %dataPathVM ='~/data/dataPsychophy/proj01_priorStrength';
            dataPathVM  = uigetdir(cd,'Pickup your project e.g., /dataPsychophy/Exp01...');
            cd(dataPathVM)
            vararginVM = [varargin,'dataPath',dataPathVM];
            
            %Load or create database
        elseif sum(strcmp(varargin,'LoadDataBase'))==0
...
if sum(strcmp(varargin,'modelPredictions'))==1
    ...
    if sum(strcmp(varargin,'withData'))==1
        ...
        if strcmp(priorShape,'vonMisesPrior')
            ...
            output.Pstd = databank.Pstd;
            output.stimStrength = databank.stimStrength;
            output.stimFeatureDeg = databank.stimFeatureDeg;
        ...
        %case we pool all subjects trials as one "big subject" and
        %calculate predictions
        if slIsInput(varargin,'poolTrialsAsOneSub')
            
            fprintf('%s \n','(SLfitCompetitionModel) subj are pooled as one "big subject"')
            
            %data
            data = round(databank.estimatesDeg);
            ...
            [meanPredtmp,stdPredtmp,cond,PestimateGivenModelUniq,...
                MAP,output] = SLmakePredictionsBayesianModel(data,d,coh,pstd,output.fitP,priorShape,...
            ...
            for j = 1 : size(output.uniqCond,1)
                ...
                if isequal(dataCondtmp,cond)
                    ...
                    if ~isequal(posThisCond,0)
                        ...
                        %output.uniqCond
                        output.meanData(j) = meanDatatmp(posThisCond);
                        output.stdData(j)  = stdDatatmp(posThisCond);
                        output.meanPred(j) = meanPredtmp(posThisCond);
                        output.stdPred(j)  = stdPredtmp(posThisCond);
                        output.PdisPred(:,j) = pPred(:,posThisCond);
                        output.PdisData(:,j) = pData(:,posThisCond);
                    else
                        %else NaN
                        output.meanData(j) = NaN;
                        output.stdData(j)  = NaN;
                        output.meanPred(j) = NaN;
                        output.stdPred(j)  = NaN;
                        output.PdisPred(:,j) = NaN(numPredEst,1);
                        output.PdisData(:,j) = NaN(numDataEst,1);
                    end
                end
            end
            
            %draw data mean and std and their models' predictions
            %output.semOfMeanData = output.stdData because all data are
            %pooled and there is no output.semOfstdData (=0).
            %same logic for predictions
            output.semOfstdData = zeros(size(output.stdData));
            output.semOfstdPred = zeros(size(output.stdPred));
            SLdrawModelsPredictionCentered(output.meanData,...
                output.stdData,output.stdData,...
                output.semOfstdData,output.meanPred,...
                output.stdPred,output.stdPred,...
                output.semOfstdPred,output.uniqCond,priorModes,...
                priorShape,'yCentered')
            
            %works only when nearby motion directions are not binned
            if ~strcmp(varargin,'binNearbyDir')
                SLConventionUp
            end
            
            %Code predictions about estimates' distributions here
            pmode = cell2mat(databank.priormodes);
            SLdrawModelsPredictionHistwithDataCentered(output.PdisData,commonSpace,...
                output.PdisPred,output.stimFeatureDeg,output.stimStrength,output.Pstd,pmode,output.uniqCond,...
                priorShape,varargin)
            
        else
            
            fprintf('%s \n','(SLfitBayesianModel) Each subj is analysed individually then subjects average is calculated"')
            ...
            for sub = 1 : numSub
                ...
                if sum(isnan(initp))==length(initp)
                    ...
                    if any(strcmp(varargin,'directoryFitParameter'))
                        ...
                        %case output present instead
                        if isempty(fitP)
                            load([inputdir,modelFold,'/',thisSubInDir{1},'/',...
                                FitfilenameThisSub(1:end-4)],'output')
                            fitP.p = output.fitP;
                        end
                ...
                %get logl
                [~,~,Logl_pertrialBestfit] = SLgetLoglBayesianModel(data,d,coh,pstd,output.fitP(sub,:),...
                    priorShape,priorModes,TheModel,varargin);
                output.negLogl = -sum(Logl_pertrialBestfit);
...
if isempty(fitPinput)==1
    ...
    if sum(strcmp(varargin{:},'FitEachKvmAndTails'))==1
        ...
        %number of each parameter types
        numLLHs = length(unique(output.StimStrength));
        numPriors = length(unique(output.pstd));
        
        %standard Bayesian model
        if ~slIsInput(varargin{:},'motionenergymodel') && length(initp)==11
            kllh       = initp(1:3);
            klearnt    = initp(4:7);
            kcardinal  = initp(8);
            fractRand  = initp(9);
            ...
            weightTail = initp(11);
            %standard Bayesian model
        elseif ~slIsInput(varargin{:},'motionenergymodel') && length(initp)~=11
            %status
            fprintf('%s \n',['(MLft5) Initial parameters are missing...',...
                'params for basic Bayesian model are:', '\n 3 kllh', '\n 4 klearnt','\n 1 kcardinal',...
                '\n 1 fractRand','\n 1 motorNoise','\n 1 weighTail'])
            keyboard
            %standard Bayesian model with motion energy
        elseif slIsInput(varargin{:},'motionenergymodel')
            %case number of parameters is correct
            if length(initp)==numPriors+4
                kllh       = [];
                klearnt    = initp(1:numPriors);
                kcardinal  = initp(numPriors+1);
                fractRand  = initp(numPriors+2);
                motorNoise = initp(numPriors+3);
                weightTail = initp(numPriors+4);
                fprintf('%s \n','Parameters are [K priors, K cardinal, prand, km and weightTail]')
                %case number of parameters is incorrect
            elseif slIsInput(varargin{:},'motionenergymodel') && length(initp)~= numPriors+4
                %status
                fprintf('%s \n',['(MLft5) Some initial parameters are missing...',...
                    'parameters for standard Bayesian model with motion energy are:','\n 4 klearnt','\n 1 kcardinal',...
                    '\n 1 fractRand','\n 1 motorNoise','\n 1 weighTail'])
                fprintf('%s \n','If there are less than 4 priors replace the missing priors with NaN')
                fprintf('%s \n','You entered more or less priors than there are in the dataset')
                fprintf('%s \n','Check the number of prior fit parameters or the datapath')
                keyboard
            end
        end
    end
    ...
    if sum(strcmp(varargin{1},'MaxLikelihoodFit'))==1
        ...
        if any(strcmp(varargin{:},'fixedPriors')) && all(klearnt)==0
            ...
            if ~slIsInput(varargin{:},'motionenergymodel')
                ...
                k0(10,:) = [repmat(nanmean([kllh klearnt]),1,numLLHs+length(klearnt)) kcardinal fractRand motorNoise weightTail];
                %nan missing parameters
                k0(10,isnan(k0(1,:))) = nan;
            else
                %(1) Input parameters (best matching parameters)
                k0(1,:) = [kllh    klearnt  kcardinal fractRand motorNoise weightTail];
                %(2) 8x stronger priors & best matching llh
            ...
            if ~any(strcmp(varargin{:},'fixedPriors'))
                if sum(strcmp(varargin{1},'FitEachKvmAndTails'))==0
                    if sum(strcmp(varargin{:},'ChangeWTail'))==0
                        ...
                        k0(9,:) = [[kllh   klearnt]./8  kcardinal fractRand motorNoise weightTail];
                        %(10)likelihoods & priors are same.
                        k0(10,:) = [repmat(nanmean([kllh klearnt]),1,length([kllh klearnt])) kcardinal fractRand motorNoise weightTail];
                    end
                end
            end
        end
    ...
    if sum(strcmp(varargin{1},'MaxLikelihoodFit'))==1
        ...
        if isempty(databankVM) || isempty(databankBim)
            ...
            %fitting options
            %debug mode (quick fit) or real fit
            if slIsInput(varargin{:},'debug')
                fprintf('%s \n','(SLfitBayesianModel) Debug: MaxIter = 1.')
                options.MaxIter = 1;
                options.MaxFunEvals = 1;
            else
                options = optimset('MaxIter',2000*size(k0,1),...
                    'MaxFunEvals',10*2000*size(k0,1));
            end
            ...
            if any(strcmp(varargin{:},'fminsearch'))
                ...
                %Max likelihood and best params
                [negLogl,position] = min(negLoglbkp);
                output.negLogl = negLogl;
                output.fitP = fitPbkp(position,:);
                output. numfitP = sum(~isnan(output.fitP));
                output.AIC = 2*(output. numfitP + output.negLogl);
                output.date = date;
